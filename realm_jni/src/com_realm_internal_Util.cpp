...
JNIEXPORT void JNICALL Java_com_realm_internal_Util_nativeSetDebugLevel(JNIEnv*, jclass, jint level)
{
    trace_level = level;
}

JNIEXPORT jlong JNICALL Java_com_realm_internal_Util_nativeGetMemUsage(JNIEnv*, jclass)
{
    return GetMemUsage();
}
    ...
    JNIEnv *env, jclass, jint testcase, jboolean dotest, jlong)
{
    ...
    switch (ExceptionKind(testcase)) {
        ...
        case IllegalArgument:
            ...
            break;
        case IOFailed:
            expect = "com.realm.IOException: Failed to open parm1. parm2";
            if (dotest)
                ThrowException(env, IOFailed, "parm1", "parm2");
            break;
        case FileNotFound:
            expect = "com.realm.IOException: File not found: parm1.";
            if (dotest)
                ThrowException(env, FileNotFound, "parm1", "parm2");
            break;
        case FileAccessError:
            expect = "com.realm.IOException: Failed to access: parm1. parm2";
            if (dotest)
        ...
        case OutOfMemory:
            expect = "com.realm.OutOfMemoryError: parm1 parm2";
            if (dotest)
                ThrowException(env, OutOfMemory, "parm1", "parm2");
            break;
        case Unspecified:
