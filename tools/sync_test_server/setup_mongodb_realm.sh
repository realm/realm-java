...
while ! curl --output /dev/null --silent --head --fail http://127.0.0.1:9090; do
  sleep 1 && echo -n .;
done;

ACCESS_TOKEN=$(curl --request POST --header "Content-Type: application/json" --data '{ "username":"unique_user@domain.com", "password":"password" }' http://127.0.0.1:9090/api/admin/v3.0/auth/providers/local-userpass/login -s | jq ".access_token" -r)
GROUP_ID=$(curl --header "Authorization: Bearer $ACCESS_TOKEN" http://127.0.0.1:9090/api/admin/v3.0/auth/profile -s | jq '.roles[0].group_id' -r)
...
curl -X PUT -d id="realm-sdk-integration-tests-$APP_ID_SUFFIX" http://127.0.0.1:8888/application-id