...
public class ViewColumnsTest extends AbstractTest {
	...
	public void shouldFindFirstRecordByColumnValue() throws IllegalAccessException {
		...
		record = employeesView.firstName.findFirst(EmployeesFixture.EMPLOYEES[1].firstName);
		assertEquals(1, record.getPosition());

		record = employeesView.salary.findFirst(EmployeesFixture.EMPLOYEES[0].salary);
		assertEquals(0, record.getPosition());

		record = employeesView.salary.findFirst(12345);
		assertNull(record);

		record = employeesView.driver.findFirst(EmployeesFixture.EMPLOYEES[0].driver);
		assertEquals(0, record.getPosition());

		record = employeesView.driver.findFirst(EmployeesFixture.EMPLOYEES[1].driver);
		assertEquals(1, record.getPosition());

		record = employeesView.birthdate.findFirst(EmployeesFixture.EMPLOYEES[1].birthdate);
		assertEquals(1, record.getPosition());

		record = employeesView.birthdate.findFirst(EmployeesFixture.EMPLOYEES[2].birthdate);
	...
	public void shouldFindAllRecordsByColumnValue() throws IllegalAccessException {
		...
		view = employeesView.firstName.findAll(EmployeesFixture.EMPLOYEES[1].firstName);
		assertEquals(1, view.size());

		view = employeesView.salary.findAll(EmployeesFixture.EMPLOYEES[0].salary);
		...
		view = employeesView.birthdate.findAll(EmployeesFixture.EMPLOYEES[2].birthdate);
		assertEquals(1, view.size());
		
		view = employeesView.birthdate.findAll(EmployeesFixture.EMPLOYEES[1].birthdate);
		assertEquals(1, view.size());
	...
	public void shouldAggregateColumnValue() {
		assertEquals(EmployeesFixture.EMPLOYEES[0].salary, employeesView.salary.minimum());
		assertEquals(EmployeesFixture.EMPLOYEES[1].salary, employeesView.salary.maximum());
	...
	public void shouldAddValueToWholeColumn() {
		...
		for (int i=0; i<EmployeesFixture.EMPLOYEES.length; ++i)
			assertEquals(EmployeesFixture.EMPLOYEES[i].salary+123, employeesView.at(i).getSalary());
	}
