...
CLASSPATH=$(pwd)/bin:$(pwd)/../lib/tightdb.jar
...
if [ -n "$JAVAC" ]; then
    ...
    for f in $FILES; do
        javac -classpath $CLASSPATH \
            ...
            $(pwd)/src/com/tightdb/examples/quickbenchmark/$f > /dev/null 2>&1
    done
...
java -Djava.library.path=$JAVA_LIB_PATH \
    -classpath $CLASSPATH \
    com.tightdb.examples.quickbenchmark.QuickBenchmark
