...
make clean || exit 1
make -j8 EXTRA_CFLAGS=-DTIGHTDB_ENABLE_REPLICATION || exit 1

# Build libtightdb-jni.so
cd "$TIGHTDB_JAVA2_HOME/tightdb_jni/src" || exit 1
make clean || exit 1
make -j8 EXTRA_CFLAGS="-I\"$JAVA_HOME/include\" -I\"$TIGHTDB_HOME/src\" -DTIGHTDB_ENABLE_REPLICATION" || exit 1
...
java -Djava.library.path="$TIGHTDB_JAVA2_HOME/tightdb_jni/src" com.tightdb.example.ReplicationExample --database-file /tmp/replication.tdb || exit 1
