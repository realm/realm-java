    ...
    JNIEnv* env, jobject, jlong nativeQueryPtr, jlongArray columnIndexes, jlong value)
{
    ...
    try {
        if (arr_len == 1) {
            ...
            Table *table = tbl.get();
            Q(nativeQueryPtr)->and_query(table->column<Int>(size_t(arr[arr_len-1])) == static_cast<int64_t>(value));
        }
    } CATCH_STD()
    if (isCopy == JNI_TRUE)
    ...
    JNIEnv *env, jobject, jlong nativeQueryPtr, jlongArray columnIndexes, jstring value, jboolean caseSensitive)
{
    ...
    try {
        ...
        if (arr_len == 1) {
            ...
            Table *table = tbl.get();
            Q(nativeQueryPtr)->and_query(table->column<String>(size_t(arr[arr_len-1])) == StringData(value2));
        }
    } CATCH_STD()
    if (isCopy == JNI_TRUE)
        env->ReleaseLongArrayElements(columnIndexes, arr, 0);
