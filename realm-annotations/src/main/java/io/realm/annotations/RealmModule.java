 ...
 * <p/>
 * A RealmModule can either be a library module or an app module. The distinction is made by setting {@code library =
 * true}. Setting {@code library = true} is normally only relevant for library authors. See below for futher details.
 * <p/>
 * <p/>
 ...
 * they now has the option of adding the library project classes to their schema using {@code
 * RealmConfiguration.addModule()}.
 *
 * @see <a href="https://github.com/realm/realm-java/tree/master/examples/realmModuleAppExample">Example of a project
 * using modules</a>
 */
...
public @interface RealmModule {
     ...
     * their own module.
     * <p/>
     * Setting both {@code allClasses = true} and {@code classes()} will result in the annotation processor throwing an
     * exception.
     */
