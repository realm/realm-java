...
/**
 * This class represents the response for a user profile values request.
 */
//FIXME this is a copy/past from AuthenticateResponse define the class when the JSON model is known from ROS TBD
public class GetUserProfileValuesResponse extends AuthServerResponse {
     ...
     */
    public static GetUserProfileValuesResponse from(Response response) {
        String serverResponse;
        try {
            serverResponse = response.body().string();
        } catch (IOException e) {
            ObjectServerError error = new ObjectServerError(ErrorCode.IO_EXCEPTION, e);
            return new GetUserProfileValuesResponse(error);
        }
        if (!response.isSuccessful()) {
            return new GetUserProfileValuesResponse(AuthServerResponse.createError(serverResponse, response.code()));
        } else {
            return new GetUserProfileValuesResponse(serverResponse);
        }
    }
     ...
     */
    public static GetUserProfileValuesResponse from(String json) {
        return new GetUserProfileValuesResponse(json);
    }

    /**
     * Helper method for creating a failed response.
     */
    public static GetUserProfileValuesResponse from(ObjectServerError error) {
        return new GetUserProfileValuesResponse(error);
    }
     ...
     */
    public static GetUserProfileValuesResponse createValidResponseWithUser(String identifier, String refreshToken, boolean isAdmin) {
        try {
            ...
            return new GetUserProfileValuesResponse(response.toString());
     ...
     */
    private GetUserProfileValuesResponse(ObjectServerError error) {
     ...
     */
    private GetUserProfileValuesResponse(String serverResponse) {
