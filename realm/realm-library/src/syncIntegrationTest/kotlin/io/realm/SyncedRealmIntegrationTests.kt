...
class SyncedRealmIntegrationTests : StandardIntegrationTest() {
    ...
    fun waitForInitialRemoteData_mainThreadThrows() {
        val user: SyncUser = SyncTestUtils.createTestUser(Constants.AUTH_URL)
        val config: SyncConfiguration = user.createConfiguration(Constants.USER_REALM)
    ...
    fun waitForInitialData_resilientInCaseOfRetries() {
        val credentials: SyncCredentials = SyncCredentials.usernamePassword(UUID.randomUUID().toString(), "password", true)
    ...
    @Throws(InterruptedException::class)
    fun defaultRealm() {
        val credentials: SyncCredentials = SyncCredentials.usernamePassword(UUID.randomUUID().toString(), "test", true)
        val user: SyncUser = SyncUser.logIn(credentials, Constants.AUTH_URL)
    ...
    fun progressListenersWorkWhenUsingWaitForInitialRemoteData() {
        ...
        SyncManager.getSession(config).addDownloadProgressListener(ProgressMode.INDEFINITELY, object : ProgressListener() {
            fun onChange(progress: Progress) {
                if (progress.isTransferComplete()) {
                    indefineteListenerComplete.set(true)
                }
            }
        })
        SyncManager.getSession(config).addDownloadProgressListener(ProgressMode.CURRENT_CHANGES, object : ProgressListener() {
            fun onChange(progress: Progress) {
                if (progress.isTransferComplete()) {
