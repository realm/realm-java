...
/**
 * An {@code OrderedRealmCollection} is a osResults which maintains an ordering for its elements. Every
 ...
 * another {@link OrderedRealmCollection}. Its size and elements order stay the same as the original osResults's when
 ...
 * </pre>
 * <p>
 * An iterator created from a live osResults will create a stable view when the iterator is created, allowing you to
...
public interface OrderedRealmCollection<E extends RealmModel> extends List<E>, RealmCollection<E> {
    ...
    /**
     * Gets the first object from the osResults.
     *
     * @return the first object.
     * @throws IndexOutOfBoundsException if the osResults is empty.
     */
    E first();

    /**
     * Gets the first object from the osResults. If the osResults is empty, the provided default will be used instead.
     *
    ...
    /**
     * Gets the last object from the osResults.
     *
    ...
    /**
     * Gets the last object from the osResults. If the osResults is empty, the provided default will be used instead.
     *
    ...
    /**
     * Sorts a osResults based on the provided field in ascending order.
     *
     ...
     * @return a new sorted {@link RealmResults} will be created and returned. The original osResults stays unchanged.
     * @throws java.lang.IllegalArgumentException if field name does not exist or it has an invalid type.
     * @throws java.lang.IllegalStateException if the Realm is closed, called on the wrong thread or the osResults is
     * an unmanaged osResults.
     */
    RealmResults<E> sort(String fieldName);

    /**
     * Sorts a osResults based on the provided field and sort order.
     *
     * @param fieldName the field name to sort by. Only fields of type boolean, short, int, long, float, double, Date,
     * and String are supported.
    ...
    /**
     * Sorts a osResults based on the provided fields and sort orders.
     *
    ...
    /**
     * Deletes the object at the given index from the Realm. This also removes it from the osResults.
     *
     ...
     * @throws UnsupportedOperationException if the osResults is unmanaged.
     */
    void deleteFromRealm(int location);

    /**
     * Deletes the first object from the Realm. This also removes it from this osResults.
     *
    ...
    /**
     * Deletes the last object from the Realm. This also removes it from this osResults.
     *
     * @return {@code true} if an object was deleted, {@code false} otherwise.
     * @throws java.lang.IllegalStateException if the Realm is closed or the method is called from the wrong thread.
     * @throws UnsupportedOperationException if the osResults is unmanaged.
     ...
     *
     * @return the snapshot of this osResults.
     * @throws java.lang.IllegalStateException if the Realm is closed or the method is called from the wrong thread.
     * @throws UnsupportedOperationException if the osResults is unmanaged.
