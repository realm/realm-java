...
public class RealmDictionary<V> extends RealmMap<String, V> {
    ...
    private static <V extends RealmModel> LinkRealmSelectorForMap<String, V> getRealmSelector(Class<V> valueClass,
                                                                                              BaseRealm baseRealm,
                                                                                              OsMap osMap) {
    ...
    private static <V> ManagedMapStrategy<String, V> getStrategy(Class<V> valueClass,
        ...
        if (isClassForRealmModel(valueClass)) {
            ...
            RealmTypeSelectorForMap<String, RealmModel> realmSelector = (RealmTypeSelectorForMap<String, RealmModel>) getRealmSelector(typeCastClass, baseRealm, osMap);
    ...
    private static <V> DictionaryManager<V> getManager(Class<V> valueClass,
                                                       BaseRealm baseRealm,
                                                       OsMap osMap) {
        RealmTypeSelectorForMap<String, V> realmTypeSelector = new RealmSelectorForMap<>(baseRealm, osMap, String.class, valueClass);

        MapValueOperator<String, ?> mapValueOperator;

        if (valueClass == Mixed.class) {
            mapValueOperator = new MixedValueOperator<>(baseRealm, osMap, (RealmTypeSelectorForMap<String, Mixed>) realmTypeSelector);
        } else if (valueClass == Long.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.LONG);
        } else if (valueClass == Float.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.FLOAT);
        } else if (valueClass == Double.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.DOUBLE);
        } else if (valueClass == String.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.STRING);
        } else if (valueClass == Boolean.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.BOOLEAN);
        } else if (valueClass == Date.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.DATE);
        } else if (valueClass == Decimal128.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.DECIMAL128);
        } else if (valueClass == Integer.class) {
            mapValueOperator = new IntegerValueOperator<>(baseRealm, osMap, (RealmTypeSelectorForMap<String, Integer>) realmTypeSelector);
        } else if (valueClass == Short.class) {
            mapValueOperator = new ShortValueOperator<>(baseRealm, osMap, (RealmTypeSelectorForMap<String, Short>) realmTypeSelector);
        } else if (valueClass == Byte.class) {
            mapValueOperator = new ByteValueOperator<>(baseRealm, osMap, (RealmTypeSelectorForMap<String, Byte>) realmTypeSelector);
        } else if (valueClass == byte[].class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.BINARY, (EqualsHelper<String, V>) new BinaryEquals<String>());
        } else if (valueClass == ObjectId.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.OBJECT_ID);
        } else if (valueClass == UUID.class) {
            mapValueOperator = new GenericPrimitiveValueOperator<>(baseRealm, osMap, realmTypeSelector, RealmMapEntrySet.IteratorType.UUID);
        } else {
    ...
    private static <V> DictionaryManager<V> getManager(String valueClass,
                                                       BaseRealm baseRealm,
                                                       OsMap osMap) {
        RealmTypeSelectorForMap<String, V> realmTypeSelector = new DynamicRealmSelectorForMap<>(baseRealm, osMap, valueClass);
