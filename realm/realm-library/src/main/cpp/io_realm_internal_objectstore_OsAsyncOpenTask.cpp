...
JNIEXPORT jlong JNICALL Java_io_realm_internal_objectstore_OsAsyncOpenTask_start(JNIEnv* env, jobject obj, jlong config_ptr)
{
    ...
    try {
        ...
        static JavaMethod java_notify_realm_ready(env, java_async_open_task_class, "notifyRealmReady", "()V", true);
        static JavaMethod java_notify_error(env, java_async_open_task_class, "notifyError", "(ILjava/lang/String;)V", true);
        ...
        task->start([task_obj=obj](std::shared_ptr<Realm> realm, std::exception_ptr error) {
        JNIEnv* local_env = jni_util::JniUtils::get_env(true);
            if (error) {
                try {
                    ...
                    local_env->CallObjectMethod(java_async_open_task_class, java_notify_error, j_error_msg);
