...
{
    add_property(env, data_ptr, j_key, util::Any(*reinterpret_cast<Row*>(row_ptr)));
}
        ...
        (JNIEnv* env, jclass, jlong data_ptr, jstring j_key, jlong list_ptr)
{
    try {
        ...
        add_property(env, data_ptr, j_key, util::Any((*list)));
        delete list;
    }
    CATCH_STD()
}
