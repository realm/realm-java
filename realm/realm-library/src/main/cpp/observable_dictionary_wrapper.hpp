...
// We need to control the life cycle of Dictionary, weak ref of Java OsMap object and the NotificationToken.
// Wrap all three together, so when the Java OsMap object gets GCed, all three of them will be invalidated.
...
void ObservableDictionaryWrapper::start_listening(JNIEnv* env, jobject j_collection_object)
{
    ...
    auto cb = [=](DictionaryChangeSet changes, std::exception_ptr err) {
        ...
        m_collection_weak_ref.call_with_local_ref(env, [&](JNIEnv* local_env, jobject collection_obj) {
            ...
            local_env->CallVoidMethod(
                    ...
                    reinterpret_cast<jlong>(changes_empty ? 0 : new DictionaryChangeSet(changes)));
        });
    };
