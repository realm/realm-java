...
public class some_test_SimpleRealmProxy extends some.test.Simple
    ...
    public void realmSet$name(String value) {
        if (proxyState.isUnderConstruction()) {
            ...
            if (value == null) {
                row.getTable().setNull(columnInfo.nameColKey, row.getColumnKey(), true);
                return;
            }
            row.getTable().setString(columnInfo.nameColKey, row.getColumnKey(), value, true);
            return;
        }
    ...
    public void realmSet$age(int value) {
        if (proxyState.isUnderConstruction()) {
            ...
            final Row row = proxyState.getRow$realm();
            row.getTable().setLong(columnInfo.ageColKey, row.getColumnKey(), value, true);
            return;
        }
    ...
    public static long insert(Realm realm, some.test.Simple object, Map<RealmModel,Long> cache) {
        ...
        if (realmGet$name != null) {
            Table.nativeSetString(tableNativePtr, columnInfo.nameColKey, rowIndex, realmGet$name, false);
        }
        Table.nativeSetLong(tableNativePtr, columnInfo.ageColKey, rowIndex, ((some_test_SimpleRealmProxyInterface) object).realmGet$age(), false);
        return rowIndex;
    }
    ...
    public static long insertOrUpdate(Realm realm, some.test.Simple object, Map<RealmModel,Long> cache) {
        ...
        if (realmGet$name != null) {
            ...
            Table.nativeSetNull(tableNativePtr, columnInfo.nameColKey, rowIndex, false);
        }
        Table.nativeSetLong(tableNativePtr, columnInfo.ageColKey, rowIndex, ((some_test_SimpleRealmProxyInterface) object).realmGet$age(), false);
        return rowIndex;
    }
