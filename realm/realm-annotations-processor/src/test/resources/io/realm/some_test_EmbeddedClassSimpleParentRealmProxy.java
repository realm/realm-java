    ...
    public static some.test.EmbeddedClassSimpleParent createOrUpdateUsingJsonObject(Realm realm, JSONObject json, boolean update)
        ...
        if (json.has("child")) {
            if (json.isNull("child")) {
                ...
                some.test.EmbeddedClass childObj = some_test_EmbeddedClassRealmProxy.createOrUpdateUsingJsonObject(realm, obj, "child", json.getJSONObject("child"), update);
            }
        }
        if (json.has("children")) {
            if (json.isNull("children")) {
                ...
                for (int i = 0; i < array.length(); i++) {
                    some.test.EmbeddedClass item = some_test_EmbeddedClassRealmProxy.createOrUpdateUsingJsonObject(realm, obj, "children", array.getJSONObject(i), update);
                }
            }
        }
        return obj;
