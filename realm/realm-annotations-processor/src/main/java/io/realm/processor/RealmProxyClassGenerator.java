...
public class RealmProxyClassGenerator {
    ...
    private void emitInitTableMethod(JavaWriter writer) throws IOException {
        ...
        writer.beginControlFlow("if (!sharedRealm.hasTable(\"" + Constants.TABLE_PREFIX + this.className + "\"))");
        writer.emitStatement("Table table = sharedRealm.getTable(\"%s%s\")", Constants.TABLE_PREFIX, this.className);
        ...
        for (VariableElement field : metadata.getFields()) {
            ...
            if (Constants.JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                String genericType = Utils.getGenericType(field);
                writer.beginControlFlow("if (!sharedRealm.hasTable(\"%s%s\"))", Constants.TABLE_PREFIX, genericType);
                writer.emitStatement("%s%s.initTable(sharedRealm)", genericType, Constants.PROXY_SUFFIX);
                writer.endControlFlow();
        ...
        writer.endControlFlow();
        writer.emitStatement("return sharedRealm.getTable(\"%s%s\")", Constants.TABLE_PREFIX, this.className);
        writer.endMethod();
        writer.emitEmptyLine();
    }
    ...
    private void emitValidateTableMethod(JavaWriter writer) throws IOException {
        ...
        for (VariableElement field : metadata.getFields()) {
            ...
            if (Constants.JAVA_TO_REALM_TYPES.containsKey(fieldTypeCanonicalName)) {
                ...
                writer.emitStatement("Table table_%d = sharedRealm.getTable(\"%s%s\")", fieldIndex, Constants.TABLE_PREFIX, genericType);
                writer.beginControlFlow("if (!table.getLinkTarget(%s).hasSameSchema(table_%d))",
                        fieldIndexVariableReference(field), fieldIndex);
