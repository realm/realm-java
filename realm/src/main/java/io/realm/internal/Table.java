...
public class Table implements TableOrView, TableSchema, Closeable {
    ...
    /**
     * Construct a Table base object. It can be used to register columns in this table. Registering into table is
    ...
    public long addEmptyRow() {
        ...
        if (hasPrimaryKey()) {
            ...
            switch (type) {
                ...
                default:
                    throw new RealmException("Cannot check for duplicate rows for unsupported primary key type: " +
                            type);
            }
        }
    ...
    public long addEmptyRows(long rows) {
        ...
        if (hasPrimaryKey()) {
            if (rows > 1) {
                throw new RealmException("Multiple empty rows cannot be created if a primary key is defined for the " +
                        "table.");
            }
            return addEmptyRow();
        }
    ...
    /**
     * Inserts a row at the specified row index. Shifts the row currently at that row index and any subsequent rows down
     * (adds one to their row index).
     */
    ...
    //Should not be called when using the dynamic interface. The methods can be accessed by calling
    // getInternalMethods() in Table class
    public class InternalMethods {
        ...
        public void insertBinary(long columnIndex, long rowIndex, ByteBuffer data) {
            ...
            if (data.isDirect())
                ...
                throw new RuntimeException("Currently ByteBuffer must be allocateDirect().");   // FIXME: support
                other than allocateDirect
        }
    ...
    /**
     * Note: The subtable returned will have to be closed again after use. You can let javas garbage collector handle
     * that or better yet call close() after use.
     *
    ...
    /**
     * Returns a non-checking Row. Incorrect use of this Row will cause a hard core crash. If error checking is
     * required, use {@link #getCheckedRow(long)} instead.
     *
    ...
    /**
     * Returns a wrapper around Row access. All access will be error checked in JNI and will throw an appropriate {@link
     * RuntimeException} if used incorrectly.
     * <p/>
     ...
     * @param columnIndex column index of the cell
     * @param rowIndex row index of the cell
     * @param data the ByteBuffer must be allocated with ByteBuffer.allocateDirect(len)
     */
    ...
    public void setBinaryByteBuffer(long columnIndex, long rowIndex, ByteBuffer data) {
        ...
        if (data.isDirect())
            ...
            throw new RuntimeException("Currently ByteBuffer must be allocateDirect()."); // FIXME: support other
            than allocateDirect
    }
     ...
     *
     * @param columnName Name of the field that will function primary key. "" or <code>null</code> will remove any
     ...
     * not having distinct values (ie. violating the primary key constraint).
     */
